...
public sealed class WorkspaceServiceTests
{
    ...
    public async Task Save_RejectsInvalidWorkspace()
    {
        ...
        try
        {
            var workspace = new MetadataStudio.Core.Domain.Workspace
            {
                WorkspaceRootPath = tempRoot,
                MetadataRootPath = Path.Combine(tempRoot, "metadata"),
                Model = new MetadataStudio.Core.Domain.ModelDefinition
                {
                    Name = "MetadataModel",
                },
                Instance = new MetadataStudio.Core.Domain.InstanceStore
                {
    ...
    public async Task Save_RejectsRelationshipCycles()
    {
        ...
        try
        {
            ...
            var entityA = new MetadataStudio.Core.Domain.EntityDefinition
            {
                Name = "EntityA",
            };
            entityA.Relationships.Add(new MetadataStudio.Core.Domain.RelationshipDefinition
            {
                Entity = "EntityB",
            });

            var entityB = new MetadataStudio.Core.Domain.EntityDefinition
            {
                Name = "EntityB",
            };
            entityB.Relationships.Add(new MetadataStudio.Core.Domain.RelationshipDefinition
            {
                Entity = "EntityA",
            });
            ...
            workspace.Instance.GetOrCreateEntityRecords("EntityA").Add(new MetadataStudio.Core.Domain.InstanceRecord
            {
                Id = "1",
            });
            workspace.Instance.GetOrCreateEntityRecords("EntityA")[0].RelationshipIds["EntityB"] = "1";

            workspace.Instance.GetOrCreateEntityRecords("EntityB").Add(new MetadataStudio.Core.Domain.InstanceRecord
            {
                Id = "1",
            });
