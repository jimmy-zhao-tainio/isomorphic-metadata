...
public sealed class LargeWorkspacePerformanceTests
{
    ...
    {
        var rowCount = ReadInt("METADATASTUDIO_PERF_ROWS", 100_000);
        var maxSaveMs = ReadInt("METADATASTUDIO_PERF_MAX_SAVE_MS", 45_000);
        var maxLoadMs = ReadInt("METADATASTUDIO_PERF_MAX_LOAD_MS", 45_000);
        var maxAllocatedMb = ReadInt("METADATASTUDIO_PERF_MAX_ALLOCATED_MB", 2_048);
    ...
    {
        if (!ReadBool("METADATASTUDIO_ENABLE_1M_PERF_TEST", defaultValue: false))
        {
            output.WriteLine("Skipping 1M-row perf test. Set METADATASTUDIO_ENABLE_1M_PERF_TEST=1 to enable.");
            return;
        }

        var maxSaveMs = ReadInt("METADATASTUDIO_PERF_1M_MAX_SAVE_MS", 300_000);
        var maxLoadMs = ReadInt("METADATASTUDIO_PERF_1M_MAX_LOAD_MS", 300_000);
        var maxAllocatedMb = ReadInt("METADATASTUDIO_PERF_1M_MAX_ALLOCATED_MB", 6_144);
