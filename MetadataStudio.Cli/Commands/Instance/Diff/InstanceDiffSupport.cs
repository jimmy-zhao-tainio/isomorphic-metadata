...
internal sealed partial class CliRuntime
{
    ...
    private const string InstanceDiffEqualModelTemplateResourceName = "MetadataStudio.Cli.Templates.InstanceDiffModel.Equal.xml";
    private const string InstanceDiffAlignedModelName = "InstanceDiffModelAligned";
    private const string InstanceDiffAlignedModelTemplateResourceName = "MetadataStudio.Cli.Templates.InstanceDiffModel.Aligned.xml";
    private const string InstanceDiffAlignmentModelName = "InstanceDiffModelAlignment";
    private const string InstanceDiffAlignmentModelTemplateResourceName = "MetadataStudio.Cli.Templates.InstanceDiffModel.Alignment.xml";
    ...
    private static readonly Lazy<string> InstanceDiffEqualModelSignature =
        new(() => ComputeModelContractSignature(InstanceDiffEqualModelTemplate.Value));
    private static readonly Lazy<string> InstanceDiffAlignedModelSignature =
        new(() => ComputeModelContractSignature(InstanceDiffAlignedModelTemplate.Value));
    private static readonly Lazy<string> InstanceDiffAlignmentModelSignature =
        new(() => ComputeModelContractSignature(InstanceDiffAlignmentModelTemplate.Value));
    ...
    private sealed record EqualDiffBuildResult(
    ...
    private sealed record AlignedDiffBuildResult(
        Workspace DiffWorkspace,
        string DiffWorkspacePath,
        bool HasDifferences,
        int LeftRowCount,
    ...
    private static ModelDefinition LoadModelTemplate(string resourceName, string expectedModelName)
    {
        ...
        var root = document.Root ?? throw new InvalidDataException($"Template '{resourceName}' has no root element.");
    ...
    private EqualDiffBuildResult BuildEqualInstanceDiffWorkspace(
        Workspace leftWorkspace,
        Workspace rightWorkspace,
        string leftWorkspacePath,
        string rightWorkspacePath)
    {
        ...
        var diffWorkspace = CreateWorkspaceFromTemplate(InstanceDiffEqualModelTemplate.Value, diffWorkspacePath);
        ...
        return new EqualDiffBuildResult(
    ...
    private AlignedDiffBuildResult BuildAlignedInstanceDiffWorkspace(
        Workspace leftWorkspace,
        Workspace rightWorkspace,
        Workspace alignmentWorkspace,
        AlignmentCatalog alignment,
        string rightWorkspacePath)
    {
        ...
        var diffWorkspace = CreateWorkspaceFromTemplate(InstanceDiffAlignedModelTemplate.Value, diffWorkspacePath);
        var identityAllocator = new IdentityAllocator();
        ...
        return new AlignedDiffBuildResult(
            DiffWorkspace: diffWorkspace,
            DiffWorkspacePath: diffWorkspacePath,
            HasDifferences: hasDifferences,
            LeftRowCount: leftSide.RowCount,
