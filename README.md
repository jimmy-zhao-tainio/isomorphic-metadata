...
- **Generic metadata model:** Plain classes (`Model`, `Entity`, `Property`, `ModelInstance`, …) with XML readers/writers.
...
| Project | Description |
| --- | --- |
| `Generic` | Core metadata types, XML reader (`Reader`), SQL reader (`ReadFromDatabase`), reflection materializer, model comparator, database instance reader. |
...
| `Samples` | Generated POCO library (built from `SampleModel.cs`). |
| `Samples.Console` | Harness that loads `SampleModel.xml` / `SampleInstance.xml`, materializes the generated classes, and prints them. |
...
4. **Round-trip back from SQL**
   ...
   - Writes updated `SampleModel.xml`, `SampleInstance.xml`.
   - Regenerates `SampleModel.cs`, `SampleModel.sql`, `SampleInstance.sql`.
